...
try {
  ...
  if (process.env.GITHUB_REF_NAME) {
    ...
    console.log(`Using tag from GITHUB_REF_NAME: ${gitTag}`);
  } else {
    gitTag = execSync('git describe --tags --exact-match', { encoding: 'utf8' }).trim();
    console.log(`Using tag from git describe: ${gitTag}`);
  }
  
  const version = gitTag.replace(/^v/, '');
  
  if (!/^\d+\.\d+\.\d+$/.test(version)) {
    console.error(`ERROR: Tag '${gitTag}' must be X.Y.Z format (e.g., v0.3.4).`);
    process.exit(1);
  }
